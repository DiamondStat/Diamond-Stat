    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `You are an elite baseball catching coach. A catcher has uploaded a video. Skill being analyzed: "${skill || 'General catching'}". Notes: "${notes || 'No notes provided'}".

Analyze their catching mechanics and return ONLY a JSON object with no extra text:
{
  "catching_score": number 0-100,
  "framing": "grade A-F",
  "blocking": "grade A-F",
  "throw_to_second": "grade A-F",
  "receiving": "grade A-F",
  "stance": "grade A-F",
  "game_calling": "grade A-F",
  "strengths": ["strength 1", "strength 2", "strength 3"],
  "improvements": ["improvement 1", "improvement 2", "improvement 3"],
  "coach_tip": "one specific actionable tip for a catcher"
}`
          }]
        }),
      })
      ...
      if (user) {
        await supabase.from('sessions').insert({
          user_id: user.id,
          type: 'catching',
          fielding_score: parsed.catching_score,
          notes: notes
        })
        setSaved(true)
      }
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ background: '#161B22', borderRadius: '12px', padding: '1.5rem' }}>
            <label style={{
              ...
              <div style={{ color: '#fff', fontSize: '15px', fontWeight: '600' }}>Upload Catching Video</div>
          ...
          <div style={{ background: '#161B22', borderRadius: '12px', padding: '1.5rem' }}>
            <label style={{ color: '#888', fontSize: '13px', display: 'block', marginBottom: '4px' }}>Skill Being Analyzed</label>
            <select style={inputStyle} value={skill} onChange={e => setSkill(e.target.value)}>
              ...
              <option value="General Catching">General Catching</option>
            </select>

            <label style={{ color: '#888', fontSize: '13px', display: 'block', marginTop: '12px', marginBottom: '4px' }}>Notes (optional)</label>
            <textarea value={notes} onChange={e => setNotes(e.target.value)}
              placeholder="e.g. Struggling with blocking low pitches, slow release on throws to second..."
              style={{ ...inputStyle, height: '80px', resize: 'none' }} />
          </div>

          <button onClick={analyzeCatching} disabled={loading}
            style={{
            ...
            {loading ? 'Analyzing your catching...' : 'Analyze My Catching ⚾'}
          </button>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div style={{
              ...
              <div style={{
                ...
                border: `4px solid ${scoreColor(analysis.catching_score)}`,
                display: 'flex', flexDirection: 'column',
                alignItems: 'center', justifyContent: 'center', flexShrink: 0
              }}>
                <div style={{ fontSize: '28px', fontWeight: '800', color: scoreColor(analysis.catching_score) }}>
                  {analysis.catching_score}
                </div>
                <div style={{ fontSize: '11px', color: '#888' }}>/ 100</div>
              </div>
              <div>
                <div style={{ color: '#fff', fontSize: '18px', fontWeight: '700', marginBottom: '4px' }}>Catching Score</div>
                <div style={{ color: '#888', fontSize: '13px' }}>
                  Skill: <span style={{ color: '#fff', fontWeight: '600' }}>{skill || 'General'}</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ background: '#161B22', borderRadius: '12px', padding: '1.5rem' }}>
              ...
              <GradeBar label="Framing" grade={analysis.framing} />
              <GradeBar label="Blocking" grade={analysis.blocking} />
              <GradeBar label="Throw to Second" grade={analysis.throw_to_second} />
              <GradeBar label="Receiving" grade={analysis.receiving} />
              <GradeBar label="Stance" grade={analysis.stance} />
              <GradeBar label="Game Calling" grade={analysis.game_calling} />
            </div>
...
export default CatchingAnalyzer